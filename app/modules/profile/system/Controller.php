    ...
    public function playerVisibilityAction()
    {
        if (isPost()) {
            ...
            if (isset($post['mid'])) {
                ...
                $response['target_h']["#".$column] = '<a class="'.$class.'" onclick="'.ajaxLoad(url('profile','playerVisibility'), 'reqest','task:'.$post['task'].'|amount:\'+ $(\'#challenge-amount\').val() +\'|mid:'.$post['mid']).'">'.Lang::translate($text);

                if($column=="looking" AND $show==0) {
                    $response['target_h']["#".$column] .= '<br />I want play for: $</a>';
        ...
        if (Request::getUri()[0]) {
            ...
            if ($match) {
                ...
                if ($match->status == 1 || ($match->status == 2 && $match->blocked)) {
                    ...
                    if (Request::getParam('user')->id == $match->uid) { //if userId is UID
            ...
            } else $response['error'] = Lang::translate("MATCH_ACCESS_DENIED");
        } else $response['error'] = Lang::translate("MATCH_ACCESS_DENIED");
                    ...
                    if ($myAssets && count($myAssets) > 0) {
                        ...
                        if (!$response['error']) {
                            ...
                            if ($match && !$match->blocked ) {
                                ...
                                } else $response['error'] = Lang::translate("MATCH_DB_ERROR");
                            } else {
                                $response['blocked'] = true;
                                $response['error'] = Lang::translate("MATCH_WRONG_OR_BLOCKED");
                            }
                        }
                    } else $response['error'] = Lang::translate("MATCH_EMPTY_MY_ASSETS");
                } else $response['error'] = Lang::translate("MATCH_WRONG_ASSETS");
            } else $response['error'] = Lang::translate("MATCH_EMPTY_ASSETS");
        }
